...
__all__ = ["FedLD", "FedLDClient", "FedLDServer"]


class MarginalLogLoss(Module):

    def __init__(self, base_loss: Module = CrossEntropyLoss(), lam: float = 0.2):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.hyper_params.update(
            ...
            loss_fn=MarginalLogLoss(base_loss=self.hyper_params.loss_fn, lam=margin_lam),
        )
...
def _get_client_grads(client_model: Module, server_model: Module) -> torch.Tensor:
    ...
    for key in server_model.state_dict().keys():
        grads.append(
            client_model.state_dict()[key].data.clone().detach().flatten()
            - server_model.state_dict()[key].data.clone().detach().flatten()
        )
    return torch.cat(grads)


def _set_client_grads(client_model: Module, server_model: Module, new_grads: torch.Tensor):
    start = 0
    for key in server_model.state_dict().keys():
        dims = client_model.state_dict()[key].shape
        end = start + dims.numel()
        client_model.state_dict()[key].data.copy_(
...
def pcgrad_svd(
    client_grads: Collection[Module], grad_history: dict, k_proportion: float = 0.1
...
class FedLDServer(Server):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            model=model, test_set=test_set, clients=clients, weighted=weighted, lr=lr, **kwargs
        )
        self.hyper_params.update(k_proportion=k_proportion)
        self.grad_history = {key: None for key in model.state_dict().keys()}
        self.grad_history["grad_len"] = {
            key: value.numel() for key, value in model.state_dict().items()
        }
    ...
    def aggregate(self, eligible: Sequence[Client], client_models: Collection[Module]) -> None:
        ...
        grad_new, self.grad_history = pcgrad_svd(
            local_clients_grads, self.grad_history, self.hyper_params.k_proportion
        )
