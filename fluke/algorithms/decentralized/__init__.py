...
class Topology:
    ...
    It is essentially a wrapper around a networkx graph, with some additional methods to
    facilitate the creation and manipulation of the graph. The nodes of the graph represent
...
class DecentralizedFL(ObserverSubject):
    ...
    This class represents the base implementation of a decentralized FL algorithm, which can be
    extended to implement specific algorithms like :class:`GossipFL`, etc.

    Args:
        n_clients (int): The number of clients in the federation.
        data_splitter (DataSplitter): The data splitter to use for splitting the dataset among
            the clients.
        hyper_params (DDict | dict[str, Any]): The hyperparameters of the algorithm.
        topology (Topology, optional): The topology of the communication graph. If not provided,
            a fully connected topology will be used by default.
        clients (list[AbstractDFLClient], optional): A list of client instances to use in the
            federation. If not provided, clients will be initialized based on the provided
            hyperparameters and topology.
        test_data (FastDataLoader | DataLoader, optional): The test dataset to be used for
            evaluation. If not provided, the test dataset will be generated by the data splitter
            along with the training datasets for the clients.
